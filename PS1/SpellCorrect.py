...
class SpellCorrect:
  ...
  def correctSentence(self, sentence):
    ...
    for i in xrange(1, len(sentence) - 1): #ignore <s> and </s>
      word = sentence[i]                  
      for editWord, editWord_probability in self.editModel.editProbabilities(word):
        tempSentence = bestSentence[:i] + [editWord] + bestSentence[i+1:]

        sentence_probability = self.languageModel.score(tempSentence)
        tempScore = sentence_probability + editWord_probability

        if tempScore >= bestScore :
          bestScore = tempScore
          max_weighted_sentence = tempSentence

    return max_weighted_sentence
