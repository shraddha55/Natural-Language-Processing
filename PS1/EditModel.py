...
class EditModel(object):
  ...
  def deleteEdits(self, word):
    ...
    for i in xrange(1, len(word)):
      ...
      #print("corruptedLetters", corruptLetters)
  ...
  def insertEdits(self, word):
    ...
    for i in xrange(1, len(word)+1):
      for j in xrange(0, len(self.ALPHABET)):
        #The corrupted signal is this character and the character preceding 
        corruptLetters = word[i-1]
        #print("corruptedLetters:", corruptLetters)
        #
        correctLetters = "%s%s" % (corruptLetters, self.ALPHABET[j])
        #print("correctLetters: ", correctLetters)
  ...
  def transposeEdits(self, word):
    ...
    for i in xrange(0, len(word)-1):
      ...
      #print("correctLetters", correctLetters)
      #The corrected word deletes character i (and lacks the start symbol)
      correction = "%s%s%s%s" % (word[1:i-1], word[i+1], word[i], word[i+2:])
      #print("correction", correction)
      ret.append(Edit(correction, corruptLetters, correctLetters))
  ...
  def replaceEdits(self, word):
    ...
    for i in xrange(1, len(word)):
      for j in xrange(0, len(self.ALPHABET)):
        ...
        correction = "%s%s%s" % (word[1:i], self.ALPHABET[j], word[i+1:])
        #print("correction: ", correction)
